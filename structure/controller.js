...
class CarController {
    ...
    renderCars(caller, carsDiv, data, searchKey) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            textSpeed.className = "text-speed";
    ...
    renderCanvasDiv(caller, raceDiv, data) {
        ...
        caller.onePix = Math.round(caller.data["distance"] / caller.canvasWidth * 100) / 100;
    ...
    draw(caller, i, image) {
        ...
        // ponovno iscravanje kolona u slucaju da su obrisane
        let part = caller.canvasWidth / caller.partNumb;
        if((x - caller.carWidth) % part == 0){
            caller.context.beginPath();
            caller.context.moveTo(x - 80, caller.canvasHeightBefore);
            caller.context.lineTo(x - 80, caller.lineHeight + caller.canvasHeightBefore);
            caller.context.stroke();
        }

        // ponovno iscrtavanje linija semafora, jer su prethodno obrisani pomeranjem automobila
        let lights = caller.data["traffic_lights"].filter(function(y) { return Math.round(y["position"]/caller.onePix) == x - caller.carWidth})[0];
        if(lights != undefined){
        ...
        let signs = caller.data["speed_limits"].filter(function(y) { return Math.round(y["position"]/caller.onePix) == x - caller.carWidth})[0];
        if(signs != undefined){
    ...
    // metoda za proveru stanja semafora
    raceLight(caller, i, image, trafficLight, speedAnimation, intervalLight){
        // ukoliko je zeleno, nastavlja sa trkom
        if(trafficLight[0]["state"]){
            ...
            // u suprotnom nastavlja da proverava stanje semafora na svakih 500 milisekondi
            setTimeout(function() {
